...
export default function KepalaTokoLayout({
  children,
}: {
      ...
      >
        ...
        <div className="p-4 flex-1">
          ...
          <nav className="space-y-1">
            {sidebarLinks.map((link) => {
              const isActive = pathname === link.href || (pathname.startsWith(link.href) && link.href !== "/dashboard/kepala-toko");
              return (
                <Link
                  key={link.name}
                  href={link.href}
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="flex items-center space-x-3 mb-4 px-2">
            <div className="w-10 h-10 rounded-full bg-pink-100 text-pink-700 flex items-center justify-center font-bold border border-pink-200 overflow-hidden">
              AS
            </div>
            <div>
              <p className="text-sm font-medium text-slate-800">Ahmad Store</p>
              <p className="text-xs text-slate-500">Kepala Toko</p>
            </div>
          </div>
