...
export default function AccountingLayout({
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 overflow-x-hidden">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-4 left-4 z-50 h-[calc(100vh-2rem)] bg-white/95 backdrop-blur-xl text-slate-800 border border-slate-200/60 rounded-3xl flex flex-col transition-all duration-300 ease-in-out ${isSidebarOpen ? "translate-x-0" : "-translate-x-[120%] xl:translate-x-0"
        ...
        <div className="p-4 flex-1 overflow-y-auto overflow-x-hidden">
          ...
          <nav className="space-y-1">
            {sidebarLinks.map((link) => {
              const isActive = pathname === link.href || (pathname.startsWith(link.href) && link.href !== "/dashboard/accounting");
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  title={isCollapsed ? link.name : undefined}
                  className={`flex items-center space-x-3 px-3 py-2.5 text-sm rounded-lg transition-all duration-200 ${isCollapsed ? "xl:justify-center xl:space-x-0 xl:px-0" : ""
                    } ${isActive
        ...
        <div className="p-4 border-t border-slate-100">
          <div className={`flex items-center space-x-3 mb-4 px-2 ${isCollapsed ? "xl:justify-center xl:space-x-0 xl:px-0" : ""}`}>
            ...
            <div className={`min-w-0 ${isCollapsed ? "xl:hidden" : ""}`}>
              <p className="text-sm font-medium text-slate-800 truncate">Budi Santoso</p>
              <p className="text-xs text-slate-500 truncate">Accounting</p>
            </div>
          </div>
